...
type TemplateData struct {
	...
	Structs                  []*Struct
	ImportPaths              []string
	TypeNameToUnderlyingType map[string]TypeInfo
...
func main() {
	...
	data := &TemplateData{
		...
		TypeNameToUnderlyingType: make(map[string]TypeInfo),
			...
			if genDecl, ok := decl.(*ast.GenDecl); ok {
				for _, spec := range genDecl.Specs {
					if typeSpec, ok := spec.(*ast.TypeSpec); ok {
						...
						case *ast.StructType:
							...
							for i, field := range t.Fields.List {
								...
								if ident, ok := field.Type.(*ast.Ident); ok {
									...
									info := extractTypeInfo(ty.String())
									structFields[i] = &StructField{
										Name: name,
										Type: info.Type,
									}
									if info.ImportPath != "" {
										data.ImportPaths = append(data.ImportPaths, info.ImportPath)
									}
									data.TypeNameToUnderlyingType[info.Type] = info
								}
							}
						...
						default:
							info := extractTypeInfo(getUnderlyingType(pkg.TypesInfo, t))
							if info.ImportPath != "" {
								data.ImportPaths = append(data.ImportPaths, info.ImportPath)
							}
							data.TypeNameToUnderlyingType[typeSpec.Name.Name] = info
						}
					}
				}
			}
...
func extractTypeInfo(s string) TypeInfo {
	...
	packageName := sp[0]
	importPath := matches[1] + "/" + packageName
	return TypeInfo{
		Package:    packageName,
...
func generateConstructor(filename string, data *TemplateData) {
	tmpl, err := template.New("constructor").Funcs(template.FuncMap{
		...
		"buildConstructorName": func(typeName string) string {
			// 外部パッケージのコンストラクタは生成しない。
			// 外部パッケージの型定義を読み込んでいないのでUnderlying Typeは元の型と常におなじになる。
			// そのため、コンストラクタを生成する必要がない。
			return data.ConstructorPrefix + toPascalCase(typeName)
		},
...
{{range .Structs}}
  ...
  func {{$prefix}}{{$structName}}({{range $index, $field := .Fields}}{{if $index}}, {{end}}{{$field.Name | camelCase}} {{ getUnderlyingType $field.Type }}{{end}}) {{if shouldReturnError}}(*{{$structName}}, error){{else}}*{{$structName}}{{end}} {
   {{range $index, $field := .Fields}}
     {{$constructorName := (buildConstructorName $field.Type)}}
