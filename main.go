...
package {{.PackageName}}
func New{{.StructName}}({{range $index, $field := .Fields}}{{if $index}}, {{end}}{{$field.Name | camelCase}} {{if isDefinedType $field.Type}}{{getRootBaseType $field.Type}}{{else}}{{$field.Type}}{{end}}{{end}}) (*{{.StructName}}, error) {
 {{range $index, $field := .Fields}}{{if isDefinedType .Type}}{{if constructorReturnsError .Type}}
 {{if isDefinedType (getBaseType .Type)}}
 tempVarByVogen{{$index}}Base, err := New{{getBaseType .Type}}({{.Name | camelCase}})
 if err != nil {
  return nil, err
 }
 tempVarByVogen{{$index}}, err := New{{.Type}}(tempVarByVogen{{$index}}Base)
 if err != nil {
  return nil, err
 }
 {{else}}
 ...
 {{else}}
 {{if isDefinedType (getBaseType .Type)}}
 tempVarByVogen{{$index}}Base := New{{getBaseType .Type}}({{.Name | camelCase}})
 tempVarByVogen{{$index}} := New{{.Type}}(tempVarByVogen{{$index}}Base)
 {{else}}
...
type raw{{.StructName}} struct {
 {{range .Fields}}
 {{.Name}} {{if isDefinedType .Type}}{{getRootBaseType .Type}}{{else}}{{.Type}}{{end}}
 {{end}}
}
